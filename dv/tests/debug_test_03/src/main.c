...
int main()
{
    ...
    uint64_t input_base = (uint64_t) fft_input; 
    uint64_t output_base = (uint64_t) fft_output;
    // each element 8 Byte
    ...
    uint8_t error = check(fft_output, fft_ref_output, N << 3);
    // check 1024 Byte
    if(error < 0){
