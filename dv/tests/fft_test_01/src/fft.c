...
void fft_load_wt(uint64_t base_addr, uint32_t stride, uint32_t tot_level){
    ...
    // write flag
    write_reg(FFT_DATA3_REG, LOAD_WT_FLAG | (tot_level << TOT_LEVEL_LOC));
    // insert instr to issue queue
    write_reg(FFT_CTRL_REG,0x1);
}

void fft_load(uint64_t base_addr, uint32_t stride, uint32_t tot_level, uint32_t mode, uint32_t lds, uint32_t ocm_addr){
    // check if fft issue queue is full
    uint32_t status = read_reg(FFT_STATUS_REG);
    // issue queue full
    while ((status & 0x1) == 0x1) {
