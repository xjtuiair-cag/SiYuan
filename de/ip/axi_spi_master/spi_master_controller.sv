...
(
  ...
  enum logic [4:0] {IDLE,CMD,ADDR,DUMMY,DATA_TX,DATA_RX,DATA_TXRX,WAIT_EDGE} state,state_next;
  ...
  begin
    ...
    case(state)
        ...
        begin
          ...
          begin
            if (spi_rd && spi_wr || spi_qrd && spi_qwr) begin
              ...
              spi_en_tx        = 1'b1;
              spi_en_rx        = 1'b1;
              state_next       = DATA_TXRX;
            end else if (spi_rd || spi_qrd)
            begin
      ...
      begin
        spi_status[4]    = 1'b1;  
        spi_cs           = 1'b0;
        spi_clock_en     = rx_clk_en || tx_clk_en;
        ctrl_data_mux    = DATA_EMPTY;
        ctrl_data_valid  = 1'b1;
        spi_en_tx        = 1'b1;
        s_spi_mode       = (en_quad) ? `SPI_QUAD_TX : `SPI_STD;
